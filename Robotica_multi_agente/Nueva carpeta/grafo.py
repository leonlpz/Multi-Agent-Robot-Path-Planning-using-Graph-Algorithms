...
class Grafo:
    ...
    def __init__(
        self, V: dict = None, E: dict = None, turtle: dict = None, colors: dict = None
    ):
        """
    ...
    def add_edge(self, vertex_o: int, vertex_i: int, weight: int):
        ...
        if (
            f"({vertex_o}, {vertex_i})" in self.E
            or f"({vertex_i}, {vertex_o})" in self.E
        ):
...
if __name__ == "__main__":
    ...
    turtle_list = (
        {}
    )  # {0: 1, 1: 4, 4: 3, 5: 'f'}  # 'f' is a centinel value to represent turtle's exit
    ...
    # Save the graph as a json file
    grafo.save_graph("graph.json")
    # print(grafo)

    # maze = Labyrinth(2,3,grafo)
    # maze.start()
